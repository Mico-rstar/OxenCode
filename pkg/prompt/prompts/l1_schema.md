# User Input
<!-- What the user said -->
**Original Request:** [Direct quote or close paraphrase]
**Intent:** [What the user actually wanted to accomplish]

# Agent Actions
<!-- What tools were called and what they accomplished -->
- **ToolName:** [Brief description of what was done and key result]
- **ToolName:** [Brief description of what was done and key result]
...

<!-- Agent's final response summary -->
**Summary:** [What the agent communicated back to the user]

# Task Status
<!-- Current state of the work -->
- **Status:** [COMPLETED | IN_PROGRESS | BLOCKED | FAILED]
- **Details:** [Brief explanation of current state]

# Key Information
<!-- High-density information worth preserving -->
<!-- Include specific file references, code snippets, errors, discoveries -->

## Files Modified
<!-- List of files that were changed with brief notes -->
- [filename](path) - [what changed]

## Key Code
<!-- Important code snippets that represent significant work -->
```
[Code snippet if relevant]
```

## Findings
<!-- Important discoveries, patterns, errors, or insights -->

## Dependencies
<!-- New dependencies, imports, or external references -->
